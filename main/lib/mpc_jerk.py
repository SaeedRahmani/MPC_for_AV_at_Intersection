...
Rd = np.diag([.3, 1.0])  # input difference cost matrix
Q_v_yaw = np.diag([0., 0.5])  # state cost matrix [v, yaw]
Qf = np.diag([1.0, 1.0, 0., 0.5, 0]) * T # state final matrix [x, y, v, yaw]
GOAL_DIS = 1.5  # goal distance
STOP_SPEED = 0.5 / 3.6  # stop speed
MAX_TIME = 13.0  # max simulation time

# Define a higher penalty for the rate of change of acceleration (jerk)
jerk_penalty_weight = 1
# Q_jerk = np.diag([jerk_penalty_weight])  # Jerk cost matrix
...
def _get_linear_model_matrix(v, phi, delta, dt, L):
    ...
    A[2, 2] = 1.0
    A[3, 3] = 1.0
    A[4, 4] = 1.0  # acceleration persists unless changed
    A[0, 2] = dt * math.cos(phi)
    A[0, 3] = - dt * v * math.sin(phi)
    A[1, 2] = dt * math.sin(phi)
    A[1, 3] = dt * v * math.cos(phi)
    A[3, 2] = dt * math.tan(delta) / L
    A[2, 4] = dt  # velocity changes by acceleration * dt]

    B = np.zeros((NX, NU))
    B[2, 0] = dt
    B[3, 1] = dt * v / (L * math.cos(delta) ** 2)
    B[4, 0] = dt  # control input directly affects the change in acceleration
    ...
    for t in range(T + 1):
        if t > 0:
            if not reaches_end[t]:
                ...
                cost += cvxpy.quad_form(xref[2:4, t] - x[2:4, t], Q_v_yaw)
            else:
        ...
        if t < (T - 1):
            ...
            cost += jerk_penalty_weight * cvxpy.square(x[4, t+1] - x[4, t])
            constraints += [cvxpy.abs(u[1, t + 1] - u[1, t]) <= MAX_DSTEER * dt]

    constraints += [x[:4, 0] == x0]
