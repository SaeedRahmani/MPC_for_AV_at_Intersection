...
Q_v_yaw = np.diag([20, 0.5])  # state cost matrix [v, yaw]
...
MAX_SPEED = 25 / 3.6  # maximum speed [m/s]
...
def _calc_ref_trajectory(state, cx, cy, cv, cyaw, dl, dt, start_idx, ov):
    ...
    xref[0, :T + 1] = cx[idx]
    xref[1, :T + 1] = cy[idx]
    # To skip the velocity - we don't need it (Q_v_yaw must have a 0 for it)
    xref[2, :T + 1] = cv[idx]
    xref[3, :T + 1] = cyaw[idx]
...
def _iterative_linear_mpc_control(x0, oa, od, state, cx, cy, cv, cyaw, dl, dt, target_ind, car_dimensions: CarDimensions):
    """
    ...
    for _ in range(MAX_ITER):
        xref, target_ind, dref, reaches_end = _calc_ref_trajectory(state, cx, cy, cv, cyaw, dl, dt, target_ind, ov)
...
class MPC:
    def __init__(self, cx: np.ndarray, cy: np.ndarray, cv: np.ndarray, cyaw: np.ndarray, dl: float, car_dimensions: CarDimensions,
                 dt: float = 0.2):
        """
        Simulation
    ...
    def set_trajectory_fromarray(self, trajectory: np.ndarray, cutoff_idx: int = 999):
        self.cx = trajectory[:, 0]
        self.cy = trajectory[:, 1]
        self.cyaw = trajectory[:, 2]
        self.cv = np.full_like(self.cyaw, MAX_SPEED)
        if cutoff_idx != 999:
            self.cv[cutoff_idx:] = 0 # this should changed to "0 from cutoff_idx to end"
    ...
    def step(self, state: State) -> Tuple[float, float]:
        ...
        self.oa, self.odelta, self.ox, self.oy, self.oyaw, self.ov, self.xref, self.target_ind = \
            _iterative_linear_mpc_control(x0, self.oa, self.odelta, state, self.cx, self.cy, self.cv,
