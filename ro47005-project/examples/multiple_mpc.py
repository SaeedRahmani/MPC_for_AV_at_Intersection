...
from envs.t_intersection_1 import t_intersection_1
...
def main():
    ...
    scenario_1 = t_intersection_1(turn_left=False)
    ...
    search_1 = MotionPrimitiveSearch(scenario_1, car_dimensions, mps, margin=car_dimensions.radius)
    
    _, _, trajectory_full = search.run(debug=False)
    _, _, trajectory_full_1 = search_1.run(debug=False)
    #########
    # INIT MPC
    #########
    ...
    mpc_1 = MPC(cx=trajectory_full_1[:, 0], cy=trajectory_full_1[:, 1], cyaw=trajectory_full_1[:, 2], dl=dl, dt=DT,
              car_dimensions=car_dimensions)
    state = State(x=trajectory_full[0, 0], y=trajectory_full[0, 1], yaw=trajectory_full[0, 2], v=0.0)
    state_1 = State(x=trajectory_full_1[0, 0], y=trajectory_full_1[0, 1], yaw=trajectory_full_1[0, 2], v=0.0)
    ...
    simulation_1 = HistorySimulation(car_dimensions=car_dimensions, sample_time=DT, initial_state=state_1)
    history_1 = simulation_1.history  # gets updated automatically as simulation runs
    ...
    for i in itertools.count():
        ...
        trajs_moving_obstacles = np.vstack(MovingObstaclesPrediction(*state_1.get(), sample_time=DT, car_dimensions=car_dimensions)
                      .state_prediction(TIME_HORIZON)).T
    ...
    traj_agent_idx_1 = 0
    tmp_trajectory_1 = None

    for i in itertools.count():
        if mpc_1.is_goal(state):
            break

        # cutoff the trajectory by the closest future index
        # but don't do it if the trajectory is exactly one point already,
        # so that the car doesn't move slowly forwards
        if tmp_trajectory_1 is None or np.any(tmp_trajectory_1[traj_agent_idx_1, :] != tmp_trajectory_1[-1, :]):
            traj_agent_idx_1 = calc_nearest_index_in_direction(state, trajectory_full_1[:, 0], trajectory_full_1[:, 1],
                                                             start_index=traj_agent_idx_1, forward=True)
        trajectory_res_1 = trajectory_1 = trajectory_full_1[traj_agent_idx:]

        # compute trajectory to correspond to a car that starts from its current speed and accelerates
        # as much as it can -> this is a prediction for our own agent
        if state.v < Simulation.MAX_SPEED:
            resample_dl = np.zeros((trajectory_res_1.shape[0],)) + MAX_ACCEL
            resample_dl = np.cumsum(resample_dl) + state.v
            resample_dl = DT * np.minimum(resample_dl, Simulation.MAX_SPEED)
            trajectory_res_1 = resample_curve(trajectory_res_1, dl=resample_dl)
        else:
            trajectory_res_1 = resample_curve(trajectory_res_1, dl=DT * Simulation.MAX_SPEED)
        ...
        #               .state_prediction(TIME_HORIZON)).T
        #     for o in moving_obstacles]
        trajs_moving_obstacles_1 = np.vstack(MovingObstaclesPrediction(*state.get(), sample_time=DT, car_dimensions=car_dimensions)
                    .state_prediction(TIME_HORIZON)).T

        # find the collision location
        collision_xy_1 = check_collision_moving_cars(car_dimensions, trajectory_res_1, trajectory_1, trajs_moving_obstacles_1,
                                                   frame_window=FRAME_WINDOW)

        # cutoff the curve such that it ends right before the collision (and some margin)
        if collision_xy_1 is not None:
            cutoff_idx_1 = get_cutoff_curve_by_position_idx(trajectory_full_1, collision_xy_1[0],
                                                          collision_xy_1[1]) - EXTRA_CUTOFF_MARGIN
            cutoff_idx_1 = max(traj_agent_idx_1 + 1, cutoff_idx_1)
            # cutoff_idx = max(traj_agent_idx + 1, cutoff_idx)
            tmp_trajectory_1 = trajectory_full_1[:cutoff_idx]
        else:
            tmp_trajectory_1 = trajectory_full_1

        # pass the cut trajectory to the MPC
        mpc_1.set_trajectory_fromarray(tmp_trajector_1)

        # compute the MPC
        delta_1, acceleration_1 = mpc_1.step(state_1)

        # show the computation results
        visualize_frame(DT, FRAME_WINDOW, car_dimensions, collision_xy_1, i, moving_obstacles, mpc_1, scenario_1, simulation,
                        state_1, tmp_trajectory_1, trajectory_res_1, trajs_moving_obstacles_1)
