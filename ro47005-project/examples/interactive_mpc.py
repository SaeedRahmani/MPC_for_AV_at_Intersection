...
def main():
    ...
    mpc = [[] for _ in ego_vehicles]
    state = [[] for _ in ego_vehicles]
    simulation = [[] for _ in ego_vehicles]
    history = [[] for _ in ego_vehicles]
    
    traj_agent_idx = [0 for _ in ego_vehicles]
    # tmp_trajectory = [None for _ in ego_vehicles]
    tmp_trajectory = [[] for _ in ego_vehicles]  

    loop_runtimes = [[] for _ in ego_vehicles]
    trajectory_res = [[] for _ in ego_vehicles]
    trajectory = [[] for _ in ego_vehicles]
    
    resample_dl = [[] for _ in ego_vehicles]
    
    collision_xy = [[] for _ in ego_vehicles]
    cutoff_idx = [[] for _ in ego_vehicles]
    trajs_other_agents = [[] for _ in ego_vehicles]
    
    delta = [[] for _ in ego_vehicles]
    acceleration = [[] for _ in ego_vehicles]
    
    # creating a list to store the location of obstacles through time for visulization
    other_agents_pos = [[] for _ in moving_obstacles]
    
    for i, ego in enumerate(ego_vehicles):
        mpc[i] = MPC(cx=paths[i][:, 0], cy=paths[i][:, 1], cyaw=paths[i][:, 2], dl=dl, dt=DT,
              car_dimensions=car_dimensions)
        state[i] = State(x=paths[i][0, 0], y=paths[i][0, 1], yaw=paths[i][0, 2], v=0.0)

        simulation[i] = HistorySimulation(car_dimensions=car_dimensions, sample_time=DT, initial_state=state[i])
        history[i] = simulation[i].history  # gets updated automatically as simulation runs
    ...
    for i in itertools.count():
        ...
        for j, ego in enumerate(ego_vehicles):
            ...
            if state[j].v < Simulation[j].MAX_SPEED:
                resample_dl[j] = np.zeros((trajectory_res[j].shape[0],)) + MAX_ACCEL
                resample_dl[j] = np.cumsum(resample_dl[j]) + state[j].v
                resample_dl[j] = DT * np.minimum(resample_dl[j], Simulation[j].MAX_SPEED)
                trajectory_res[j] = resample_curve(trajectory_res[j], dl=resample_dl[j])
            else:
                trajectory_res[j] = resample_curve(trajectory_res[j], dl=DT * Simulation[j].MAX_SPEED)
        ...
        visualize_frame(DT, FRAME_WINDOW, car_dimensions, collision_xy, i, other_agents, mpc, mpc[0].create_scenario(), simulation,
                        state, tmp_trajectory, trajectory_res, trajs_other_agents)

        # move all obstacles forward
        for i_obs, o in enumerate(other_agents):
            other_agents_pos[i_obs].append((i,(state[k].x, state[k].y))) # i is time here
            o.step()

        # step the simulation (i.e. move our agent forward)
        for i_ego, ego in enumerate(ego_vehicles):
            state[i_ego] = simulation.step(a=acceleration[i_ego], delta=delta[i_ego], xref_deviation=mpc[i_ego].get_current_xref_deviation())
